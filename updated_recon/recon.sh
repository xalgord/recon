...
run_subfinder() {
    ...
    if check_tool "subfinder"; then
        subfinder -d "$domain" -o "$output_dir/passive/subfinder.txt" -silent 2>/dev/null
...
run_findomain() {
    ...
    if check_tool "findomain"; then
        findomain -t "$domain" -u "$output_dir/passive/findomain.txt" 2>/dev/null
    ...
    if [[ $total_discovered -gt 0 ]]; then
        ...
        if check_tool "httpx" && [[ -f "$output_dir/final/validated_subdomains.txt" ]]; then
            ...
            if [[ $validated_count -gt 0 ]]; then
                ...
                cat "$output_dir/final/validated_subdomains.txt" "$output_dir/bruteforce/dnsx_bruteforce.txt" "$output_dir/passive/*.txt" | \
                httpx -threads "$threads" -title -status-code -tech-detect -follow-redirects \
